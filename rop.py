...
class ROP():
    ...
    def set_variables(self):
        ...
        self.theta = self.model.addVars(self.pn.bus_num, self.r_num, lb=-math.pi/6, ub=math.pi/6,
    ...
    def set_constraints(self):
        ...
        for k in range(self.r_num):
            for i in range(self.pn.load_num):
                ...
                self.model.addGenConstrIndicator(self.z[i + self.pn.bus_num + self.pn.load_num, k], False,
        ...
        for k in range(self.r_num):
            for i in range(self.pn.branch_num):
                ...
                self.model.addGenConstrIndicator(self.z[i + self.pn.bus_num + self.pn.gen_num + self.pn.load_num, k],
                                                 False, self.pl[i, k] == 0, name=con_name)
        ...
        for i in range(self.pn.branch_num):
            ...
            for k in range(self.r_num):
                ...
                self.model.addGenConstrIndicator(self.z[i + self.pn.bus_num + self.pn.gen_num + self.pn.load_num, k],
                                                 True, self.pl[i, k] ==
                                                 self.pn.branch[i][4] * 100 * (self.theta[to_bus, k] -
                                                                               self.theta[from_bus, k]))
    ...
    def display_o(self):
        ...
        for i in self.repair:
            if i in self.damaged_node:
                value = []
                for j in range(self.r_num):
                    var_name = "y[" + str(i) + "," + str(j) + "]"
                    var = self.model.getVarByName(var_name)
                    value.append(var.x)
